...
bool confirm(const string& question) {
    ...
    string r;
    getline(cin, r);
    return !r.empty() && (r[0]=='o' || r[0]=='O' || r[0]=='y' || r[0]=='Y');
}
    ...
    while (running) {
        ...
        switch (choice) {
            ...
            case 2: { // Remove Book
                ...
                if (Book* b = library.findBookByISBN(isbn)) {
                    cout << "\nVous allez supprimer :\n" << b->toString() << "\n";
                    if (confirm("Confirmer la suppression")) {
                        if (library.removeBook(isbn)) {
                            cout << "Livre supprimé avec succès !\n";
                        } else {
                            cout << "Une erreur est survenue lors de la suppression.\n";
                        }
                    } else {
            ...
            case 9: { // Check Out Book
                ...
                User* u = library.findUserById(userId);
                 if (b && u) {
                fileManager.appendBorrowLog(u->getUserId(), u->getName(),
                                        b->getISBN(), b->getTitle(), "EMPRUNT");
                                                }
                } else {
            ...
            case 10: { // Return Book
                ...
                Book* b = library.findBookByISBN(isbn);
                    string userId = "INCONNU";
                 string userName = (b ? b->getBorrowerName() : "INCONNU");

                if (library.returnBook(isbn)) {
                    cout << "Livre retourné avec succès !\n";
                            fileManager.appendBorrowLog(userId, userName,
                                    b ? b->getISBN() : isbn,
                                    b ? b->getTitle() : string("Titre inconnu"),
                                    "RETOUR");
            ...
            case 14: { // Afficher tous les livres triés par TITRE
                library.displayAllBooksByTitle();
                pauseForInput();
                break;
}

            case 15: { // Afficher tous les livres triés par AUTEUR

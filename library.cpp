...
vector<Book*> Library::searchBooksByTitle(const string& title) {
    ...
    for (auto& book : books) {
        ...
        transform(bookTitle.begin(), bookTitle.end(), bookTitle.begin(),
                  [](unsigned char c){ return std::tolower(c); });
...
vector<Book*> Library::searchBooksByAuthor(const string& author) {
    ...
    for (auto& book : books) {
        ...
        transform(bookAuthor.begin(), bookAuthor.end(), bookAuthor.begin(),
                  [](unsigned char c){ return std::tolower(c); });
...
vector<Book*> Library::getAllBooksByAuthor() {
    vector<Book*> allBooks;
    for (auto& book : books) {
        allBooks.push_back(book.get());
    }

    // Tri : auteur alphabétique, puis titre
...
void Library::displayAllBooksByTitle() {
    ...
    sort(sorted.begin(), sorted.end(), [](const Book* a, const Book* b) {
        return a->getTitle() < b->getTitle();  // tri par TITRE uniquement
    });
...
void Library::displayAllBooksByAuthor() {
    ...
    sort(sorted.begin(), sorted.end(), [](const Book* a, const Book* b) {
        return a->getAuthor() < b->getAuthor(); // tri par AUTEUR uniquement
    });
